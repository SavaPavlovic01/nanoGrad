...
template<typename T1>
void mult_constant_kernel_opencl(cl_mem a, double value, size_t n) {
    auto& context = OpenCLContext::get(); 
    std::string kernel_name = std::format("mult_constant_{}", OpenCLContext::type_to_cl_string<T1>());
    ...
    T1 mult_val = static_cast<T1>(value);
    clSetKernelArg(kernel, 0, sizeof(cl_mem), &a);
    clSetKernelArg(kernel, 1, sizeof(T1), &mult_val);
    clSetKernelArg(kernel, 2, sizeof(size_t), &n);
