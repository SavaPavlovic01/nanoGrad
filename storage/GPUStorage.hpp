...
public:
    ...
    std::shared_ptr<Storage> cross_entropy(const std::shared_ptr<Storage>& targets, const std::vector<uint32_t> shape) override {
        ...
        auto kernel = context.get_kernel_by_name("softmax_cross_entropy_naive");
        if(!kernel.has_value()) {
            throw std::runtime_error("WTF");
        }

        cl_mem dest = context.allocateBuffer(batches * sizeof(float), CL_MEM_READ_WRITE);
        clSetKernelArg(kernel.value(), 0, sizeof(cl_mem), &data);
